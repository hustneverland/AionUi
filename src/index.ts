...
const createTray = (): void => {
  ...
  const iconPath = app.isPackaged ? path.join(process.resourcesPath, iconFile) : path.join(process.cwd(), 'resources', iconFile);
  ...
  // Resize for tray (16x16 on Windows/Linux, macOS handles automatically)
...
const handleAppReady = async (): Promise<void> => {
  ...
  if (isResetPasswordMode) {
    ...
    // Intercept close to minimize to tray on Windows/Linux
    if (process.platform !== 'darwin') {
    ...
    // Auto-start WebUI from persisted config
    try {
      const webuiConfig = await ProcessConfig.get('webui.config');
      ...
      console.error('[App] Failed to auto-start WebUI from saved config:', error);
    }
  }
...
app.on('window-all-closed', () => {
  // Don't quit when tray is active or in WebUI mode
  if (!isWebUIMode && !tray && process.platform !== 'darwin') {
    app.quit();
  }
});
